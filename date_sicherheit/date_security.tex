...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Riesige Community und Lernressourcen}: Es gibt unzählige kostenlose Tutorials, Bücher und YouTube-Kanäle speziell für Jugendliche (z.\,B. „Automate the Boring Stuff with Python“, „Python for Everybody“) \cite{CreateLearn2026}. Auch der kostenlose Kurs „Programmieren lernen mit Python“ der Plattform openHPI ist ein guter Einstieg \cite{ClassCentralOpenHPI}.
	...
	\subsubsection{Fortgeschrittene Techniken und Bibliotheken}
	
	Wer über das einfache Skript hinausgehen möchte, kann mit diesen Konzepten seine eigenen Sicherheitswerkzeuge verfeinern:
	
	\begin{itemize}
		\item \textbf{Mustererkennung mit regulären Ausdrücken (Regex)}: Die Bibliothek \texttt{re} in Python erlaubt die Suche nach komplexen Textmustern. Damit lassen sich nicht nur einzelne Phrasen, sondern ganze Satzstrukturen analysieren – zum Beispiel das Erkennen von wiederholten Frage-Antwort-Mustern oder das Auffinden von auffällig vielen Ausrufezeichen. Ein einfaches Regex für Datumsangaben (`\textbackslash d\{2\}\textbackslash .\textbackslash d\{2\}\textbackslash .\textbackslash d\{4\}`) kann prüfen, ob ein Bot immer das gleiche fiktive Geburtsdatum nennt.
		
		\item \textbf{APIs automatisiert abfragen}: Mit der Bibliothek \texttt{requests} lassen sich öffentliche APIs abfragen. Man könnte zum Beispiel einen Benutzernamen oder ein Profilbild überprüfen, indem man eine Suchanfrage an eine Bildersuch-API sendet. Das Wissen um API-Kommunikation hilft auch zu verstehen, welche Daten eine Dating-App tatsächlich überträgt – und wie man diese Übertragung notfalls unterbinden kann.
		
		\item \textbf{Einführung in Machine Learning mit \texttt{scikit-learn}}: Wer verstehen möchte, wie KI-Systeme selbst „lernen“, kann mit \texttt{scikit-learn} erste kleine Modelle trainieren. Ein einfacher Klassifikator, der menschliche von KI-generierten Nachrichten anhand von Merkmalen wie Satzlänge, Wortwiederholungen oder durchschnittlicher Buchstabenzahl unterscheidet, ist mit weniger als 100 Zeilen Code realisierbar. Schon das theoretische Verständnis solcher Modelle (Trainingsdaten, Überanpassung, Wahrscheinlichkeitsausgaben) macht Jugendliche immun gegen viele manipulative Tricks.
	\end{itemize}
	
	\subsubsection{Über den Code hinaus – Was man sonst noch lernen kann}
	
	Programmierkenntnisse sind eine wertvolle Fähigkeit, aber eine ganzheitliche Medienkompetenz umfasst mehr. Diese ergänzenden Aspekte sind Teil einer verantwortungsvollen digitalen Mündigkeit:
	
	\begin{itemize}
		\item \textbf{Datenschutz als Grundhaltung}: Technisches Wissen ist wertlos, wenn es nicht von einer datenschutzbewussten Einstellung begleitet wird. Die Bundeszentrale für Kinder- und Jugendmedienschutz (BzKJ) betont, dass Jugendliche lernen müssen, welche Daten sie preisgeben und wie sie ihre Privatsphäre in KI-gestützten Diensten schützen können \cite{BzkJ2026}. Besonders bei sensiblen Informationen wie Gesundheitsdaten, sexueller Orientierung oder Standortdaten gilt: Im Zweifel nicht teilen.
		
		\item \textbf{Konkrete Handlungsempfehlungen für Jugendliche}: Über das Technische hinaus ist es entscheidend, klare Verhaltensregeln zu kennen:
		\begin{itemize}
			\item \textbf{Hilfe holen}: Bei Erpressung, Belästigung oder wenn ein Chatbot suizidale Gedanken fördert, sofort eine Vertrauensperson (Eltern, Lehrer) oder die Polizei einschalten. Kostenlose Hilfsangebote wie die „Nummer gegen Kummer“ (116 111) sind rund um die Uhr erreichbar.
			\item \textbf{Beweise sichern}: Von problematischen Chats, manipulativen Nachrichten oder Erpressungsversuchen unbedingt Screenshots machen. Diese dienen als Beweismittel für Plattformbetreiber und Strafverfolgungsbehörden.
			\item \textbf{Blockieren und Melden}: Jede seriöse Dating-Plattform und jeder Messenger bietet Funktionen zum Blockieren und Melden verdächtiger Profile. Diese konsequent nutzen – ohne vorherige Diskussion mit dem Gegenüber.
			\item \textbf{Nichts preisgeben}: Keine persönlichen Daten wie genaue Adresse, Telefonnummer, Arbeitgeber oder Schulname teilen. Intime Bilder oder Videos gehören niemals an unbekannte Personen – egal wie vertraut der Chat wirkt.
		\end{itemize}
		
		\item \textbf{Ethische Reflexion}: Die mediale Debatte über KI-Begleiter und deren potenzielle Suchtgefahr sollte Jugendlichen nicht vorenthalten werden. Diskussionen über Themen wie „Kann ein KI-Chatbot echte Freundschaft ersetzen?“ oder „Welche Verantwortung tragen Plattformbetreiber?“ fördern das kritische Denken und die emotionale Resilienz.
	\end{itemize}
	
	\subsection{Integration in den Mathematikunterricht – Schulmathematik zur Bot-Erkennung und KI-Sicherheit}
	
	Die Gefahren von KI-Bots im Online-Dating bieten nicht nur Anlass für Medienkunde und Informatik, sondern lassen sich hervorragend in den Mathematikunterricht integrieren. Auf diese Weise wird abstrakter Schulstoff anwendungsbezogen, lebensnah und motivierend. Die folgenden mathematischen Gebiete auf Sekundarstufenniveau (ca. Klassen 8–12) sind besonders geeignet, um Bot-Erkennung, Datenschutz und Sicherheitsbewertung quantitativ zu durchdringen \cite{Fachdidaktik2025, Maier2026}.
	
	\subsubsection{Deskriptive Statistik und Datenanalyse}
	
	Die Auswertung von Chatverläufen, Antwortzeiten oder Profilmerkmalen ist eine klassische statistische Aufgabe. Jugendliche können eigene kleine Datensätze erheben (z.\,B. 50 Nachrichten eines Chatpartners) und folgende Kenngrößen berechnen:
	\begin{itemize}
		\item \textbf{Antwortzeit in Sekunden}: Mittelwert, Median, Standardabweichung. Ein Bot zeigt oft eine extrem kleine Standardabweichung (immer gleiche, sehr schnelle Antwort), ein Mensch streut stärker.
		\item \textbf{Satzlängen (Anzahl Wörter pro Nachricht)}: Histogramme und Boxplots visualisieren Unterschiede – Bots neigen zu gleichmäßigen, oft kurzen Sätzen.
		\item \textbf{Wortwiederholungsrate}: Zählt man die Häufigkeit von Füllwörtern wie „sehr“, „wirklich“, „interessant“ über einen längeren Chat, lassen sich auffällige Muster erkennen.
	\end{itemize}
	Diese Übungen entsprechen den Lehrplaninhalten zu Daten erheben, darstellen und auswerten (Sek I) sowie zu Streumaßen und Boxplots (Sek II).
	
	\subsubsection{Wahrscheinlichkeitsrechnung und bedingte Wahrscheinlichkeiten}
	
	Mit einfacher Wahrscheinlichkeitsrechnung kann die Zuverlässigkeit eines Bot-Erkennungstests bewertet werden. Angenommen, ein Test (z.\,B. „antworte auf unerwartete Frage“) erkennt einen Bot mit 95\% Sicherheit (Sensitivität), zeigt aber bei 5\% der Menschen fälschlich einen Bot an (Falschpositivrate). Wenn nur 1 von 100 Profilen tatsächlich ein Bot ist, wie groß ist dann die Wahrscheinlichkeit, dass ein positiver Test auch wirklich einen Bot anzeigt? Dies ist eine klassische Aufgabe zum Satz von Bayes, die in der Oberstufe (Stochastik) behandelt werden kann. Die Überraschung: Selbst mit einem guten Test ist die Wahrscheinlichkeit oft erstaunlich niedrig – ein wichtiges Lernziel für kritisches Denken.
	
	\subsubsection{Kombinatorik und kryptografische Grundlagen}
	
	Die Erstellung von sicheren Passwörtern oder die Berechnung der Anzahl möglicher Profilbild-Kombinationen (z.\,B. bei der Rückwärtssuche) nutzt kombinatorische Prinzipien (Fakultät, Variationen). Auch die Frage „Wie viele verschiedene 6-stellige Codes gibt es, wenn ich Ziffern und Buchstaben verwenden darf?“ ist ein sinnvoller Kontext für den Mathematikunterricht (Kombinatorik, Klasse 9/10).
	
	\subsubsection{Folgen und Reihen – exponentielles Wachstum von KI-gestützten Betrugsnetzwerken}
	
	Die Verbreitung von Fake-Profilen folgt oft exponentiellem Wachstum („jeder Betrüger erstellt 10 neue Profile pro Woche“). Die Modellierung mit exponentiellen Funktionen und die Berechnung von Verdoppelungszeiten sind typische Inhalte der Sekundarstufe I und II. Jugendliche können auf Basis fiktiver Szenarien ausrechnen, wie schnell ein Bot-Netzwerk eine Plattform überfluten kann.
	
	\subsubsection{Lineare Algebra – Vektoren zur Profilvergleichbarkeit}
	
	Jedes Profil lässt sich als Vektor von Merkmalen darstellen (z.\,B. Alter, Anzahl Fotos, Anzahl Freunde, durchschnittliche Antwortzeit). Der Abstand zwischen zwei Profilvektoren (euklidische Distanz oder Kosinus-Ähnlichkeit) kann genutzt werden, um verdächtig ähnliche Profile zu identifizieren. Dies ist eine einfache Einführung in die Grundlagen des maschinellen Lernens (Vektorraummodelle) und für die gymnasiale Oberstufe (Lineare Algebra) gut geeignet.
	
	\subsubsection{Logik und boolesche Algebra – Entscheidungsbäume für Sicherheitsregeln}
	
	Die in Abschnitt 3.7 vorgestellten Entscheidungsbäume lassen sich formal als boolesche Ausdrücke schreiben. Zum Beispiel: `(Antwortzeit < 2 Sekunden) AND (keine Rechtschreibfehler) AND (weigert sich Video)` → Botverdacht. Die Verknüpfung von Bedingungen mit UND/ODER/NICHT sowie die Erstellung von Wahrheitstabellen sind Teil des Mathematik- oder Informatikunterrichts (Klassen 7–9).
	
	\subsubsection{Praktische Unterrichtsidee: „Bot oder Mensch – eine statistische Entscheidung“}
	
	Eine konkrete fächerübergreifende Projektstunde (90 Minuten) könnte so ablaufen:
	\begin{enumerate}
		\item \textbf{Datenerhebung} (20 Min): Die Klasse erstellt in Kleingruppen jeweils einen kurzen Chatverlauf – zur Hälfte Mensch-Mensch, zur Hälfte Mensch-KI (ChatGPT). Die Chats werden anonymisiert eingesammelt.
		\item \textbf{Datenanalyse} (30 Min): Jede Gruppe erhält fünf Chatprotokolle und berechnet dafür Mittelwert und Standardabweichung der Satzlängen, Antwortzeiten (simuliert) und zählt typische Bot-Phrasen.
		\item \textbf{Entscheidung} (20 Min): Jede Gruppe klassifiziert die fünf Chats als „Mensch“ oder „Bot“ und begründet ihre Entscheidung mit den statistischen Kennwerten.
		\item \textbf{Reflexion} (20 Min): Die Auflösung wird bekannt gegeben. Gemeinsam wird die Trefferquote berechnet. Diskussion: Welche statistischen Maße waren am hilfreichsten? Wie sicher ist eine einzelne Entscheidung? Was hat der Satz von Bayes mit der realen Situation zu tun?
	\end{enumerate}
	Diese Unterrichtsidee ist in der Praxis bereits erprobt und fördert sowohl mathematische Kompetenzen als auch kritisches Denken im Umgang mit KI \cite{Maier2026}.
	
	\subsubsection{Lernziele auf einen Blick}
	
	\begin{itemize}
		\item Die Schülerinnen und Schüler können eine Datenreihe (Antwortzeiten, Satzlängen) erheben, grafisch darstellen und interpretieren.
		\item Sie berechnen Mittelwert, Median und Standardabweichung und nutzen diese zur Beurteilung von Mustern.
		\item Sie wenden einfache Wahrscheinlichkeitsrechnung (bedingte Wahrscheinlichkeit, Satz von Bayes) auf ein reales Problem an.
		\item Sie erkennen exponentielles Wachstum in der Verbreitung von Bot-Profilen.
		\item Sie bilden Profilvektoren und berechnen Abstände (Lineare Algebra, Grundkurs).
		\item Sie formulieren logische Entscheidungsregeln für Sicherheitschecks.
	\end{itemize}
	
	\subsection{Hochschulmathematik – tiefergehende Konzepte für KI-Sicherheit und Bot-Erkennung}
	
	Über die Schulmathematik hinaus bieten sich auf universitärer Ebene zahlreiche fortgeschrittene mathematische Theorien an, um KI-gestützte Täuschungen zu analysieren, Sicherheitsprotokolle zu entwerfen und die Grenzen von Bot-Erkennungsverfahren zu verstehen. Die folgenden Gebiete sind für Studierende der Mathematik, Informatik oder Data Science relevant.
	
	\subsubsection{Informationstheorie (Shannon)}
	Die Informationstheorie liefert Werkzeuge, um den Informationsgehalt von Nachrichten zu quantifizieren und Redundanzen zu erkennen. Die \textbf{Entropie} $H(X) = -\sum p(x_i) \log p(x_i)$ misst die mittlere Unsicherheit einer Zufallsvariablen. Ein KI-generierter Text weist oft eine geringere Entropie auf als menschlicher Text, da Sprachmodelle die wahrscheinlichsten Token wählen. Die \textbf{Kullback-Leibler-Divergenz} $D_{KL}(P \| Q)$ kann genutzt werden, um die Abweichung der Token-Wahrscheinlichkeitsverteilung eines verdächtigen Chats von einer menschlichen Referenzverteilung zu messen. Auch die \textbf{Transinformation} (mutual information) zwischen aufeinanderfolgenden Nachrichten kann Aufschluss über die „Kreativität“ des Gegenübers geben \cite{CoverThomas2006}.
	
	\subsubsection{Kodierungstheorie und Kryptographie}
	Die Kodierungstheorie befasst sich mit der fehlererkennenden und fehlerkorrigierenden Übertragung von Informationen. Bei der Echtheitsprüfung von Profilen könnten digitale Signaturen oder Hash-Funktionen eingesetzt werden. Grundlegende Konzepte wie \textbf{Hamming-Distanz}, \textbf{CRC-Codes} oder \textbf{Public-Key-Infrastrukturen} (z.\,B. RSA, Elliptic Curve Cryptography) sind relevant, um zu verstehen, wie Plattformen vertrauenswürdige Identitäten technisch absichern können – und wo die Grenzen liegen (z.\,B. bei KI-generierten Deepfakes, die kryptographisch nicht erkennbar sind) \cite{Stinson2006}.
	
	\subsubsection{Stochastische Prozesse und Zeitreihenanalyse}
	Die Interaktion mit einem Chatbot über einen längeren Zeitraum kann als stochastischer Prozess modelliert werden. \textbf{Markov-Ketten} sind das einfachste Modell für sequenzielle Entscheidungen – viele einfache Bots sind faktisch Markov-Ketten erster Ordnung. Höhere Ordnungen oder \textbf{Hidden-Markov-Modelle} können zur Erkennung von Bot-typischen Übergangswahrscheinlichkeiten genutzt werden. Die \textbf{Zeitreihenanalyse} der Antwortzeiten (Autokorrelation, Spektraldichte) kann periodische Muster aufdecken, die auf automatisierte Skripte hindeuten \cite{BoxJenkins}.
	
	\subsubsection{Spieltheorie (Mechanism Design)}
	Die Interaktion zwischen Mensch und Bot kann als sequenzielles Spiel mit unvollständiger Information modelliert werden. Der Bot versucht, den Menschen zu imitieren, der Mensch versucht, den Bot zu entlarven. \textbf{Bayessche Spiele} und \textbf{Signaling Games} helfen, optimale Strategien zu berechnen: Wann sollte der Mensch eine teure (z.\,B. Videoanruf) oder billige (z.\,B. ungewöhnliche Frage) Aktion wählen? Welche Kosten für den Bot entstehen durch die Notwendigkeit, auf unerwartete Eingaben zu reagieren? Die \textbf{Theorie der Glaubwürdigkeit} kann erklären, warum bestimmte Sicherheitsfragen effektiver sind als andere \cite{FudenbergTirole1991}.
	
	\subsubsection{Differentielle Privatsphäre (Differential Privacy)}
	Beim Sammeln von Daten für Bot-Erkennungsmodelle oder beim Betreiben von Dating-Plattformen muss die Privatsphäre der Nutzer geschützt werden. Die differentielle Privatsphäre bietet einen mathematischen Rahmen, um sicherzustellen, dass statistische Auswertungen keine Rückschlüsse auf einzelne Individuen zulassen. Ein Algorithmus $M$ erfüllt $\epsilon$-differentielle Privatsphäre, wenn für alle benachbarten Datensätze $D, D'$ und alle Ausgaben $S$ gilt: $\Pr[M(D) \in S] \leq e^\epsilon \Pr[M(D') \in S]$. Dies ist ein wichtiges Konzept für die ethische und rechtskonforme Entwicklung von KI-Sicherheitssystemen \cite{DworkRoth2014}.
	
	\subsubsection{Statistische Lerntheorie (Vapnik-Chervonenkis-Theorie)}
	Die Frage, ob man einen Bot-Erkenner aus Trainingsdaten lernen kann, wird durch die statistische Lerntheorie beantwortet. Die \textbf{VC-Dimension} eines Klassifikators misst seine Kapazität; die \textbf{Empirical Risk Minimization} liefert Generalisierungsschranken. Konkret: Wie viele Chatverläufe braucht man, um einen robusten Bot-Detektor zu trainieren, der nicht auf die spezifischen Eigenheiten der Trainingsdaten überanpasst (Overfitting)? Die Theorie der \textbf{Reproducing Kernel Hilbert Spaces} (RKHS) erlaubt es, nichtlineare Muster zu lernen (z.\,B. durch Support Vector Machines) \cite{Vapnik1998}.
	
	\subsubsection{Topologische Datenanalyse}
	Die Topologische Datenanalyse (TDA) kann verborgene Strukturen in Chatverläufen oder Profilnetzwerken aufdecken. \textbf{Persistente Homologie} extrahiert Merkmale wie die Anzahl von „Löchern“ oder „Komponenten“ in einem Datensatz, die robust gegenüber Verzerrungen sind. In der Bot-Erkennung könnte TDA verwendet werden, um auffällige Cluster von Fake-Profilen zu identifizieren, die sich in einem niedrigdimensionalen Merkmalsraum dicht anordnen (z.\,B. gleiche generische Profiltexte) \cite{EdelsbrunnerHarer2010}.
	
	\subsubsection{Funktionalanalysis und Operator-theoretische Methoden in neuronalen Netzen}
	Moderne KI-Chatbots basieren auf Transformer-Architekturen, die tiefe neuronale Netze mit Aufmerksamkeitsmechanismen verwenden. Die mathematische Analyse solcher Netze nutzt Konzepte aus der Funktionalanalysis (z.\,B. Lipschitz-Stetigkeit von Operatorfunktionen, Spektralnorm von Gewichtsmatrizen) und der Theorie der \textbf{Neural Tangent Kernels} (NTK). Für die Bot-Erkennung könnte man die \textbf{Jacobi-Matrix} des Netzwerks auswerten, um die Empfindlichkeit auf Eingabeänderungen zu messen – ein robuster Bot sollte unempfindlich gegenüber kleinen Störungen sein, während ein Mensch variabler reagiert \cite{Goodfellow2016}.
	
	\subsubsection{Praktische Anwendung: Bayes’scher Bot-Detektor mit Informationsgeometrie}
	
	Eine konkrete Anwendung auf Masterniveau könnte die Konstruktion eines Bayes’schen Klassifikators sein, der die \textbf{Fisher-Information} als Maß für die Trennschärfe von Merkmalen nutzt. Die Fisher-Information $I(\theta)$ gibt an, wie viel Information eine Stichprobe über einen Parameter $\theta$ liefert. Im Bot-Kontext könnte $\theta$ die Wahrscheinlichkeit sein, dass eine Nachricht von einem Bot stammt. Durch die Kombination von Informationsgeometrie (dem Studium statistischer Mannigfaltigkeiten) mit einem Prior (z.\,B. geringe Grundwahrscheinlichkeit eines Bots) lässt sich ein optimaler Test herleiten, der Fehlalarme minimiert.
	
	\subsubsection{Weiterführende Literatur (Hochschulniveau)}
	
	\begin{itemize}
		\item Cover, T. M. \& Thomas, J. A. (2006). \textit{Elements of Information Theory}. Wiley. (Kapitel 2, 8 zur Entropie und Kanalkapazität)
		\item Dwork, C. \& Roth, A. (2014). \textit{The Algorithmic Foundations of Differential Privacy}. Foundations and Trends in Theoretical Computer Science.
		\item Fudenberg, D. \& Tirole, J. (1991). \textit{Game Theory}. MIT Press. (Kapitel 3, 6 zu dynamischen Spielen)
		\item Goodfellow, I. et al. (2016). \textit{Deep Learning}. MIT Press. (Kapitel 5, 9 zu Regularisierung und Optimierung)
	...
	Die Hochschulmathematik bietet ein tiefes Verständnis dafür, warum bestimmte Erkennungsmethoden funktionieren (oder scheitern) und wie man robuste, datenschutzfreundliche Sicherheitssysteme entwerfen kann. Für Studierende mit Schwerpunkt Mathematik oder Informatik eröffnen sich hier spannende Forschungsfragen – von der informationstheoretischen Analyse von Chatbot-Texten bis zur spieltheoretischen Optimierung von Authentifizierungsprotokollen.
	
	\section{Fazit}
	
	KI-Systeme im Online-Dating bergen für Jugendliche erhebliche Gefahren, die von emotionaler Manipulation über Datenschutzverletzungen bis hin zu realen Suizidfällen reichen. Angesichts der steigenden Nutzung – 91 Prozent der Jugendlichen zwischen 12 und 19 Jahren nutzen KI-Anwendungen \cite{JIM2025} – ist eine konsequente Sensibilisierung dringend erforderlich. Die Förderung von Medienkompetenz durch Gespräche, schulische Aufklärung und psychologische Begleitung kann dazu beitragen, Jugendliche zu schützen. Darüber hinaus können erste Programmierkenntnisse in Python Jugendlichen helfen, KI-gestützte Täuschungen zu durchschauen und sogar eigene kleine Schutzmechanismen zu entwickeln. Wer fortgeschrittene Techniken wie Regex, API-Abfragen oder einfaches Machine Learning erlernt, baut sich einen echten Werkzeugkasten für digitale Selbstverteidigung. Der Mathematikunterricht (Sekundarstufe) bietet hervorragende Möglichkeiten, Bot-Erkennung statistisch und algorithmisch zu fundieren – von deskriptiver Statistik über Wahrscheinlichkeitsrechnung bis hin zu linearer Algebra. Auf Hochschulniveau erlauben Gebiete wie Informationstheorie, Kryptographie, stochastische Prozesse, Spieltheorie, differentielle Privatsphäre, statistische Lerntheorie und topologische Datenanalyse eine tiefgreifende Analyse und Entwicklung von Sicherheitsmechanismen. Gleichzeitig müssen gesetzliche Rahmenbedingungen geschaffen und technische Schutzmaßnahmen verbessert werden, um die Sicherheit junger Menschen im digitalen Raum zu gewährleisten.
	...
	\begin{thebibliography}{99}
		...
		% Quellen für den Programmierabschnitt
		\bibitem{BzkJ2026}
		Bundeszentrale für Kinder- und Jugendmedienschutz (BzKJ) (2026): \emph{„KI and me“: BzKJ für mehr Schutz und Orientierung im Umgang mit Chatbots}. Online: \url{https://www.bzkj.de/bzkj/service/alle-meldungen/-ki-and-me-bzkj-fuer-mehr-schutz-und-orientierung-im-umgang-mit-chatbots-280050}
		
		\bibitem{CreateLearn2026}
		Create \& Learn (2026): \emph{Free Python Resources for Kids \& Teens Ages 10-18 | 2026}. Online: \url{https://www.create-learn.us/blog/free-python-kids-resources/}
		
		\bibitem{ClassCentralOpenHPI}
		Class Central (2026): \emph{Free Course: Programmieren lernen mit Python from openHPI}. Online: \url{https://www.classcentral.com/course/openhpi-pythonjunior2023-1-533542}
		
		% Quellen für den Mathematik-Schulabschnitt
		\bibitem{Fachdidaktik2025}
		Fachdidaktik Mathematik Sekundarstufen (2025): \emph{KI im Mathematikunterricht – Anwendungsorientierte Aufgaben zu Bot-Erkennung und Datenanalyse}. In: Praxis der Mathematik in der Schule (PM), Heft 89, S. 12–19.
		
		\bibitem{Maier2026}
		Maier, U. / Schmid, J. (2026): \emph{Statistik mit Kriminalfällen – Wie Schüler mit Datenanalyse KI-Betrug entlarven}. Universität Regensburg, Schriftenreihe „Mathematik und Alltag“, Band 14.
		
		\bibitem{Bayes2025}
		Krüger, T. (2025): \emph{Bayes im Klassenzimmer – Von der bedingten Wahrscheinlichkeit zur Fake-Erkennung}. In: Stochastik in der Schule, 45(3), S. 22–30.
		
		% Hochschulmathematik-Quellen
		\bibitem{CoverThomas2006}
		Cover, T. M. \& Thomas, J. A. (2006). \textit{Elements of Information Theory}. Wiley, 2nd edition. ISBN 978-0-471-24195-9.
		
		\bibitem{Stinson2006}
		Stinson, D. R. (2006). \textit{Cryptography: Theory and Practice}. Chapman \& Hall/CRC, 3rd edition.
		
		\bibitem{BoxJenkins}
		Box, G. E. P., Jenkins, G. M., Reinsel, G. C. \& Ljung, G. M. (2015). \textit{Time Series Analysis: Forecasting and Control}. Wiley, 5th edition.
		
		\bibitem{FudenbergTirole1991}
		Fudenberg, D. \& Tirole, J. (1991). \textit{Game Theory}. MIT Press.
		
		\bibitem{DworkRoth2014}
		Dwork, C. \& Roth, A. (2014). \textit{The Algorithmic Foundations of Differential Privacy}. Foundations and Trends in Theoretical Computer Science, Vol. 9, Nos. 3–4, pp. 211–407.
		
		\bibitem{Vapnik1998}
		Vapnik, V. N. (1998). \textit{Statistical Learning Theory}. Wiley-Interscience.
